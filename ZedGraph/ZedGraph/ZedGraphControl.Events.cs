        ...
        protected void ZoomScale(Axis axis, double zoomFraction, double centerVal, bool isZoomOnCenter) {
            if (axis != null && zoomFraction > 0.0001 && zoomFraction < 1000.0) {
				...
				*/
                ...
                double range = (maxLin - minLin) * zoomFraction;

                double f = 0.5;
                if (isZoomOnCenter) {
                    f = (centerVal - minLin) / (maxLin - minLin);
                }

                axis._scale._minLinearized = centerVal - range * f;
                axis._scale._maxLinearized = centerVal + range * (1 - f);
                //				}
